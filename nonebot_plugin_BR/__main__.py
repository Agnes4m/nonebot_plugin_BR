		...
		if data_path.is_file():
				...
				if game_data["player_id2"]:
						...
						if player_id in [game_data["player_id"], game_data["player_id2"]]:
								...
								if game_data["round_self"] and player_id == game_data[
										"player_id"]:
										game_players.append(
														cast(
																		"PlayerSession",
								...
								if (
												not game_data["round_self"] and player_id == game_data[
										"player_id"]
										...
										await ai_do(game_data, game_state, matcher, session_uid,
																session, -1)
										game_players.append(
														cast(
																		"PlayerSession",
...
async def _(
		...
		if not game_data["player_id2"]:
				await matcher.finish(
								"你还没有对手呢,快艾特你的好朋友发送“br加入”进入游戏吧")
		
		# 首次攻击判定
		if not game_data["is_start"]:
				logger.info("[br]开始游戏,先手为player1")
				game_data, _, new_weapon1, new_weapon2 = await Weapon.new_item(
								game_data)
...
"""
		...
		if not game_data["round_self"] and player_id == game_data["player_id"]:
				await matcher.finish(
								f"现在是{game_data['player_name2']}的回合\n请等待对手行动")
...
async def ai_do(
		...
		action = await ai_action(game_data, game_sate,
		...
		if action:
				if action.action_type == "开枪":
						...
						if if_reload:
								await matcher.send(out_msg)
						if target == "2":
								game_data, out_msg = await Game.start(game_data,
																											True)  # noqa: FBT003
						else:
								game_data, out_msg = await Game.start(game_data,
																											False)  # noqa: FBT003
						...
						if "handcuffs" in txt:
								...
								if not state_data["is_finish"]:
										await ai_do(game_data, state_data, matcher, session_uid,
																session, 2)
								else:
										return
...
async def _(
		...
		session_uid = session.get_id(SessionIdType.GROUP)
		game_data = await LocalData.read_data(session_uid)
		if player_id != game_data["player_id"] and player_id != game_data[
				"player_id2"]:
...
async def _(
		...
		if "刀" in txt:
				...
				game_data = await Weapon.use_knife(game_data)
				game_data[t_items]["knife"] -= 1
				await LocalData.save_data(session.get_id(SessionIdType.GROUP),
																	game_data)
...
async def _(
		...
		# 结束游戏并清理玩家
		game_players[:] = [one for one in game_players if
											 one["session_uid"] != session_uid]
		await LocalData.delete_data(session_uid)
